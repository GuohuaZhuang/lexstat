...
#define DICT_PG_E       "../data/PG_E.DICT"
...
// OK
int list_map_push(
    LS_LISTMAP* listmap, const char* sword, const char* desc, DICT_TYPE type) {
    if (!sword || !desc) { fprintf(stderr, "list_map_push err\n"); return -1; }
    if (!listmap) {
        listmap = (LS_LISTMAP*) malloc(sizeof(LS_LISTMAP));
        if (!listmap) { fprintf(stderr, "malloc error!\n"); return -1; }
        memset(listmap, 0, sizeof(LS_LISTMAP));
    }
    if (!listmap->map) { listmap->map = cbmapopen(); }
    int _fv = 1;
    cbmapput(listmap->map, sword, -1, (const char*)&_fv, sizeof(_fv), 0);
    
    LS_SWORD* _ls = (LS_SWORD*) malloc(sizeof(LS_SWORD));
    if (!_ls) { fprintf(stderr, "malloc error!\n"); return -1; }
    memset(_ls, 0, sizeof(LS_SWORD));
    _ls->sword = strdup(sword); _ls->desc = strdup(desc); _ls->type = type;
    if (!listmap->list) { listmap->list = _ls; }
    else {
        _ls->next = listmap->list;
        listmap->list->prev = _ls;
        listmap->list = _ls;
    }
    return 0;
}
...
int db_search(
    ...
    return vlget(db, sword, -1, NULL) ? 1 : 0;
}
...
int lexstat_statistic(LEXSTAT* lexstat, const char* s) {
    ...
    for (_sdup = sdup; (word = strtok_r(_sdup, delim, &_ssave)); _sdup = NULL) {
        sword = lexstat_stemming(lex, word);
